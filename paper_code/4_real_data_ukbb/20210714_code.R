...
get.res.test <- function(x1,lam1,n.gwas,a,rough,twostage){
  m <- length(x1)
  if(twostage){
  ...
  idx <- (x1^2<=s2thld)
  if(!rough){inf=F}
  temp1 <-  calH2.new2(x1[idx], lam1[idx],   N=n.gwas,a=a, rough=rough,inf=inf)$a
    a <- max(temp1,0)
    h2 <- calH2.new2(x1, lam1,   N=n.gwas, a=max(temp1,0), rough=rough)$h2 ##?
  }else{
    h2 <- calH2.new2(x1, lam1,   N=n.gwas, a=a, rough=rough)$h2
    a <- calH2.new2(x1, lam1,   N=n.gwas, a=a, rough=rough)$a
  }
...
get.res <- function(x1,lam1,n.gwas,a,rough,twostage){
  ...
  if(twostage){
    ...
    #if(!rough){inf=F}
    if(rough){
    temp1 <-  calH2.new1(x1, lam1,   N=n.gwas,a=a, rough=rough)$a
    }else{
    ...
    est <- calH2.new1(x1, lam1,   N=n.gwas, a=a, rough=rough)
    h2 <- est$h2
    a <- est$a
  }
  return(list(a=a,h2=h2))
