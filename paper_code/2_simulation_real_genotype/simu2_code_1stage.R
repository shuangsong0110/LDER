...
for (n in c(5000,1e4,2e4,5e4)){
  ...
  res.ukb <- mclapply(1:50,run.result,n=n,alpha=alpha,ld.type='ukb',mc.cores=2,twostage=F) ## cores not too large, 10 cores inside
...
for (n in c(2000,5000,1e4,2e4,5e4)){
  res.ukb <- mclapply(1:50,run.result,n=n,alpha=alpha,ld.type='1kg',mc.cores=2,twostage=F,mc.preschedule=F)
  lder11 <- ldsc11 <- lder11.sd <- ldsc11.sd <- c()
  for(k in 1:50){
    lder11[k] <- res.ukb[[k]]$lder$h2
    ldsc11[k] <- res.ukb[[k]]$ldsc$h2
